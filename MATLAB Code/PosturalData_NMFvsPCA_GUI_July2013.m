...
global output_signal DATAsrc DATA_LST checkwalk time nmus ncond ntrials nbins color
...
if checkwalk==1
    load muscledata.mat DATA_LST DATAsrc time %eventually modify this to allow user to select a file
else
    load example_postural_data output_signal DATA_LST
    nmus=14; ncond=12; ntrials=5; nbins=4;
...
[nmus,nconds]=size(DATAsrc);
    ...
    for s=1:5
        ...
        if synergy_nmf(s).VAF <= 95
            num_of_synergy_at_threshold=s;
        end

      
        fprintf('Varience accounted for  %d synergy:  %d.\n', s, synergy_nmf(s).VAF);%print values in the command window



    end
    save("posturaldatavalues.mat","num_of_synergy_at_threshold","synergy_nmf")
    %------------------LK added code_end----------------------------------------------------------
